...
def data_distrib(npar, data, scaling_factor):

    if data == 'mixture':
        init_sample = torch.randn(npar, 2)
        p = init_sample.shape[0] // 2
        init_sample[:p, 0] = init_sample[:p, 0] - scaling_factor
        init_sample[p:, 0] = init_sample[p:, 0] + scaling_factor
...
def two_dim_ds(npar, data_tag, scaling_factor):
    init_sample = data_distrib(npar, data_tag, scaling_factor)
    ...
    return init_ds, mean_per_dim, var_per_dim
