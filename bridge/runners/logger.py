...
class CSVLogger(Logger):
    ...
    def __init__(self, source, destination, directory='./', name='logs', save_stride=1):
        edge_directory = directory + 'source={source},dest={destination}/'.format(source=source,
                                                                                  destination=destination)
        self.logger = _CSVLogger(edge_directory, name=name)
        self.count = 0
